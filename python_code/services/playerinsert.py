...
def cache_db_entries(session):
    return (
        {c.competition_name: c.competition_id for c in session.query(Competition).all()},
        {t.team_name: t.team_id for t in session.query(Teams).all()},
        {c.country_name: c.country_id for c in session.query(country).all()}
    )
