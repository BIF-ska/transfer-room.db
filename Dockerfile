...
# Add cron job for running scheduler.py at midnight
RUN echo "0 0 * * * python3 /app/scheduler.py >> /var/log/cron.log 2>&1" > /etc/cron.d/scheduler_cronjob
RUN chmod 0644 /etc/cron.d/scheduler_cronjob
RUN crontab /etc/cron.d/scheduler_cronjob

# Ensure cron starts in the foreground
